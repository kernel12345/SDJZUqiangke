...
- [一、项目背景](#一项目背景)
- [二、功能清单](#二功能清单)
- [三、接口技术规格（2026-09-16 实地抓包验证）](#三接口技术规格2026-09-16-实地抓包验证)
  - [3.1 公选课选课接口 `ggxxkxkOper`](#31-公选课选课接口-ggxxkxkoper)
  - [3.2 课程列表查询接口 `xsxkGgxxkxk`](#32-课程列表查询接口-xsxkgxxkxk)
  - [3.3 其他类别选课接口](#33-其他类别选课接口)
  - [3.4 选课按钮 HTML 结构](#34-选课按钮-html-结构)
  - [3.5 批次 ID `jx0502zbid`](#35-批次-id-jx0502zbid)
- [四、脚本架构](#四脚本架构)
  - [4.1 模块划分](#41-模块划分)
  - [4.2 调用链](#42-调用链)
- [五、关键机制详解](#五关键机制详解)
  - [5.1 类别识别（三级兜底）](#51-类别识别三级兜底)
  - [5.2 课程关键词 → 内部 ID 自动补全](#52-课程关键词--内部-id-自动补全)
  - [5.3 抢课循环](#53-抢课循环)
  - [5.4 静默 confirm / alert](#54-静默-confirm--alert)
  - [5.5 HTTP 请求双兜底](#55-http-请求双兜底)
- [六、安装指南](#六安装指南)
- [七、使用指南](#七使用指南)
...
│  │  [GXYS9474] 经典电影赏析 (韩洁) 余0           ☐    │        │
│  │  [GXGL9497] 数据艺术 (徐茜) 余52              ☐    │        │
│  │  …更多                                           │        │
│  │  [🔄 读取页面课程] [全选] [清空]                  │        │
│  └─────────────────────────────────────────────────┘        │
│                                                             │
│  ③ 输入课程编号/名称/教师，一键加入目标 ✨ 核心功能 ✨         │
│  ┌─────────────────────────────────────────────────┐        │
│  │  每行一个，例如:                                 │        │
│  │  GXGL9500                                       │        │
│  │  数据艺术                                       │        │
...
| 区域 | 功能 | 操作 |
|------|------|------|
| ① 按条件搜索 | 调查询接口按课程名/课程号/教师/星期/节次筛选 | 填关键词 → 点"搜索" |
| ② 已识别课程 | 当前页面/查询到的所有课程，勾选加入抢课目标 | 勾选复选框 / 全选 / 清空 |
| ③ 输入课程关键词 | ✨ 最常用 ✨ 输入课程编号/名称/教师，自动查接口补全 ID | 每行一个 → 点"🔎 查接口并加入目标" |
| ④ 抢课参数 | 控制循环行为 | 修改数字框 |
| ⑤ 日志 | 实时显示每次请求的结果 | 滚动查看 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| 重试间隔 | 800ms | 两次请求之间的等待时间。建议 ≥ 500ms，太容易被风控 |
